 ...
 * Module dependencies
 */
const { isPlainObject, isString, isUndefined, merge, omit, camelCase } = require('lodash');
...
module.exports = {
  ...
  parseCriteria(req) {
    ...
    req.options.criteria.blacklist = req.options.criteria.blacklist || ['limit', 'skip', 'sort', 'populate', 'filter'];
    ...
    let where = req.allParams().where || req.allParams().filter;

    if(where && where.id && where.id.indexOf(',') !== -1) {
      where.id = where.id.split(',');
    }
    ...
    values = Object.assign(
      ...
      Object.keys(relationships).reduce((acc, key) => {
        ...
        if(Array.isArray(data)) {
          // this relation has many items so extract the IDs
          return Object.assign({}, acc, { [camelCase(key)]: data.map(d => d.id) });
        }
        return data && data.id ? Object.assign({}, acc, { [camelCase(key)]: data.id }) : acc;
      }, {})
    );
  ...
  parseSkip(req) {
    ...
    const page = req.param('page');
    if(page) {
      const pageNumber = typeof page === 'object' ? page.number : page;
      const limit = this.parseLimit(req);
      // subtract one from the page as they are not 0 based
      return pageNumber && limit ? (pageNumber - 1) * limit : 0;
    }
